...
def process_external_document(url: str, usuario_id: int, version: str = "1.0"):
    ...
    try:
        ...
        # 9) Detectar tipo de documento y clasificación
        ...
        # 10) Detectar MIME type si está disponible
        tipo_mime = response.headers.get("Content-Type", "")
        if mime:
            try:
                detected_mime = mime.from_file(str(storage_path))
                if detected_mime:
                    tipo_mime = detected_mime
            except Exception:
                pass
